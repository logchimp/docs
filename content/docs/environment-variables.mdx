...
Deployment with [one-click deploy](/docs/install/getting-started#one-click-deploy) buttons by-default pre-fills most of the environment variables during the deployment process; only a few of the environment variables need to be configured manually, such as secret credentials.
